...
panel_summary_before = pd.pivot_table(runs, values = 'seq_name', columns='panel_name', 
...
# heme versioning
'''
First heme sample on current panel is CPDC150846 anything with a number greater 
than that is current panel and anything below it is old panel (last one on old 
panel CPDC150841)
'''
...
# solid versioning
'''
Final solid v.1 (FFPE 1) is CPDV162807, First solid v2 is CPDC162815
'''
reads.loc[(reads.panel_name == 'Solid') &
          (reads.year >= 16) &
          (reads.acc_num > 2807), 'panel_version'] = 2

reads.loc[(reads.panel_name == 'Solid') &
          (reads.year < 16), 'panel_version'] = 1
          
reads.loc[(reads.panel_name == 'Solid') &
          (reads.year >= 16) &
          (reads.acc_num <= 2807), 'panel_version'] = 1
